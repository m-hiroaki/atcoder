...
int main()
{
    ...
    for (int i = 0; i < n; i++){
        ...
        if(a==1){
            start.first = max(x, start.first);
        }
        else if(a==2)
        {
            end.first = min(x, end.first);
        }
        else if(a==3)
        {
            start.second = max(y, start.second);
        }
        else if(a==4)
        {
            end.second = min(y, end.second);
        }
    }

    cout << max((end.first - start.first), (ll)0) * max((end.second - start.second), (ll)0) << endl;
    return 0;
}